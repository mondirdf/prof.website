    ...
    <div className="pt-32 pb-20 min-h-screen bg-purple-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            <h2 className="text-2xl font-bold text-purple-900 mb-6">الملفات</h2>
            <div className="space-y-4">
              {lesson.files.map((file) => (
                <Card key={file.id} hover={false} className="flex items-center justify-between">
                  <div>
                    <h4 className="font-bold text-purple-900">{file.name}</h4>
                    <p className="text-sm text-purple-600">{file.size}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <button className="text-purple-900 hover:text-purple-700 flex items-center gap-2">
...
const FilesPage = () => {
  ...
  return (
    <div className="pt-32 pb-20 min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-purple-900 mb-4">مكتبة الملفات</h1>
          <p className="text-lg text-purple-600">حمّل ملخصات، تمارين وامتحانات سابقة.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {files.map((f) => (
            <Card key={f.id} className="flex flex-col justify-between">
              <div>
                <h3 className="text-lg font-bold text-purple-900 mb-2">{f.name}</h3>
                <p className="text-sm text-purple-600 mb-4">{f.category} • {f.size}</p>
              </div>
              <div className="flex items-center justify-between mt-4">
                <span className="text-sm text-purple-500">{f.downloads} تحميل</span>
                <a href={f.url} className="text-purple-900 hover:underline flex items-center gap-2">
...
const VideosPage = () => {
  ...
  return (
    <div className="pt-32 pb-20 min-h-screen bg-purple-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-purple-900 mb-4">الفيديوهات</h1>
          <p className="text-lg text-purple-600">شروحات ومراجعات مسجلة للمقررات.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {videos.map((v) => (
            <Card key={v.id} hover={false}>
              <h3 className="text-lg font-bold text-purple-900 mb-4">{v.title}</h3>
              <div className="aspect-video bg-purple-100 rounded-lg overflow-hidden">
                <iframe
                  title={v.title}
                  src={v.url}
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  ...
  return (
    <div className="pt-32 pb-20 min-h-screen bg-white">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-purple-900 mb-4">آراء الطلاب</h1>
          <p className="text-lg text-purple-600">بعض مراجعات التلاميذ حول طريقة التدريس.</p>
        </div>
    ...
    <div className="pt-32 pb-20 min-h-screen bg-purple-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div>
            <h1 className="text-4xl font-bold text-purple-900 mb-4">من هو الأستاذ محمد؟</h1>
            <p className="text-lg text-purple-600 mb-4">
              أستاذ رياضيات مع خبرة طويلة في تحضير التلاميذ لامتحانات البكالوريا. يعتمد أسلوباً واضحاً ومنظماً مع أمثلة تطبيقية وتمارين محلولة.
            </p>
...
const ContactPage = ({ onMessageSent }) => {
  ...
  const submit = async (e) => {
    e.preventDefault();
    setSending(true);
    // placeholder for submitMessage -> replace with real API call
    ...
    <div className="pt-32 pb-20 min-h-screen bg-white">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-purple-900">تواصل معنا</h1>
          <p className="text-purple-600">أرسل لنا طلبك وسنعاود الاتصال بك في أقرب وقت.</p>
        </div>

        <form onSubmit={submit} className="bg-purple-50 p-8 rounded-xl shadow-md">
          <Input label="اسم الطالب" placeholder="أدخل اسمك" value={name} onChange={(e) => setName(e.target.value)} required />
          <Input label="رقم الهاتف" placeholder="+212 6..." value={phone} onChange={(e) => setPhone(e.target.value)} required />
          <Textarea label="الرسالة" placeholder="اكتب رسالتك هنا..." value={message} onChange={(e) => setMessage(e.target.value)} required rows={6} />
          <div className="text-right">
            <Button type="submit" onClick={() => {}} icon={<Mail size={16} />}>
              {sending ? 'جاري الإرسال...' : 'أرسل الرسالة'}
            </Button>
          </div>
          {success && <p className="mt-4 text-green-600">تم إرسال رسالتك بنجاح — سنرد عليك قريبًا.</p>}
        </form>
      </div>
    </div>
  );
...
const App = () => {
  ...
  // Use scroll animation hook to activate AOS-like classes (placeholder usage)
  useScrollAnimation();
